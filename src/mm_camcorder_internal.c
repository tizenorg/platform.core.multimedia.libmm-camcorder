...
static void state_callback(mrp_res_context_t *context, mrp_res_error_t err, void *user_data);
/* callback for resource set update */
static void resource_callback(mrp_res_context_t *cx, const mrp_res_resource_set_t *rs, void *user_data);
/* synchronus callback for resource conflict scenario */
static void resource_release_cb (mrp_res_context_t *cx, const mrp_res_resource_set_t *rs, void *user_data);
/* thread for murphy resource manager initialization*/
static void mrp_routine(void *user_data);
		...
		if (res) {
			if (MRP_RES_RESOURCE_ABOUT_TO_LOOSE == res->state) {
				if ( ret = _mmcamcorder_stop((MMHandleType)hcamcorder) != MM_ERROR_NONE) {
					_mmcam_dbg_err("resource_release_cb:Camera preview failed to stop");
				}

				if (hcamcorder->mrp_data.rs) {
					if ( ret = mrp_res_release_resource_set(hcamcorder->mrp_data.rs) != MM_ERROR_NONE) {
						_mmcam_dbg_err("resource_release_cb:Resource set fail to release");
					}
				}
				break;
			// Murphy requests to free the resource.
			// The app should stop all the operations that use this resource.
			// E.g. stop playing audio or close opened files.
			}
		}
	}
...
static void mrp_routine(void* user_data)
{
	...
	hcamcorder->mrp_data.cx = mrp_res_create(ml, state_callback, (void*)(hcamcorder));
...
int _mmcamcorder_start(MMHandleType handle)
{
	...
	//Add the required resources in resource set before starting the camera preview pipeline
