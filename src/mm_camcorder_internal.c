...
void _mmcamcorder_dpm_mic_policy_changed_cb(const char *name, const char *value, void *user_data)
{
	mmf_camcorder_t *hcamcorder = MMF_CAMCORDER(user_data);
	int current_state = MM_CAMCORDER_STATE_NONE;
	int ret = MM_ERROR_NONE;
	...
	_mmcam_dbg_warn("mic policy [%s], current state [%d]", value, current_state);

	if (!strcmp(value, "disallowed") && current_state >= MM_CAMCORDER_STATE_RECORDING) {
		_MMCAMCORDER_LOCK_ASM(hcamcorder);
